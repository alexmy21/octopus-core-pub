...
//    public static final int MEASURE_COEFFICIENT_DEFAULT = 1;
    ...
    private static final Integer    PROD_ABS_LOW        = 0;
    ...
    private IntegerExpressionVariable[] resourceIntExpVars = null;
...
//    private int measureCoefficient; 
    ...
    public ChocoOptiModel(
        ...
        this.domainOption           = Options.V_BOUND;
    ...
    public IntegerExpressionVariable    createResourceTotalIntExtVariable(){        
        return Choco.sum(getResourceIntExpVariables());
    }
    ...
    private IntegerVariable[] createIntVarPlanArray() {
        ...
        for (Iterator<Constant> it = dataList.iterator(); it.hasNext();) {
            ...
            intVars[i] = Choco.makeIntVar(name, (int)lowbound, (int)upperbound, getDomainOption());
            i++;
        }
        return intVars;
    }
    ...
    private IntegerVariable[] createIntVarDataAbsArray() {
        ...
        for(OptiBean item : dataList){
            ...
            intVars[i] = Choco.makeIntVar(name, (int)lowbound, 
                    (int)upperbound, getDomainOption());
...
//            String name = entry.getKey();
    ...
    public Model addPlanConstraints(Model chocoModel) {
        ...
        for (int i = 0; i < intVar.length; i++) {
            ...
            for (int j = 0; j < expVars.length; j++) {
                switch(getPlanType()){
                    case ChocoOptiModel.PLAN_TYPE_AGGRESSIVE:
                        chocoModel.addConstraint(Choco.geq(expVars[j], intVar[i]));
                        break;
                    case ChocoOptiModel.PLAN_TYPE_BALANCED:
                        chocoModel.addConstraint(Choco.eq(expVars[j], intVar[i]));
                        break;
                    case ChocoOptiModel.PLAN_TYPE_CAUTIOUS:
    ...
    public Model addResourceConstraints(Model chocoModel) {
        ...
        IntegerExpressionVariable[] expVars = this.getResourceIntExpVariables();
...
//            chocoModel.addVariable(expVar);
            chocoModel.addConstraint(Choco.leq(expVars[i], intVar[i]));
        }
    ...
    private IntegerExpressionVariable[] getResourceIntExpVariables() {
        
        if (resourceIntExpVars == null) {
            List<Constant> resources = this.getResourceConstantList();
            this.resourceIntExpVars = new IntegerExpressionVariable[resources.size()];

            int i = 0;
            for (Constant item : resources) {
                IntegerExpressionVariable resource = createResourceIntExpVariable(item.getName());
                resourceIntExpVars[i] = resource;
                i++;
            }
        }
        
        return resourceIntExpVars;
    }
    ...
    /**
     * @param domainOption the domainOption to set
     */
