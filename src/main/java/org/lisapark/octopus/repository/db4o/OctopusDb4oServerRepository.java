 ...
 */
public class OctopusDb4oServerRepository  extends AbstractOctopusRepository implements OctopusRepository {

    private static final Logger LOG = LoggerFactory.getLogger(OctopusDb4oServerRepository.class);
    ...
    public OctopusDb4oServerRepository(ObjectServer server) {
        this.server = server;      
    }
    
    public OctopusDb4oServerRepository(String fileName) {
        init(fileName, 0, "", "");
    }
   
    public OctopusDb4oServerRepository(String fileName, int port) {
        init(fileName, port, "", "");        
    }
    
    public OctopusDb4oServerRepository(String fileName, int port, String uid, String psw) {
    ...
    private void init(String fileName, int port, String uid, String psw) {
        ...
        if (isFileUnlocked) {
            ...
            this.server = Db4oClientServer.openServer(config, fileName, port);
            server.grantAccess(uid, psw);
            
            this.container = server.openClient();
        } else {
            this.container = Db4oClientServer.openClient(fileName, port, uid, psw);
        }
    }
