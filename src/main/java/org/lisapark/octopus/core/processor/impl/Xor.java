 ...
 */
@Persistable
public class Xor extends Processor<Void> {
//    private static final String DEFAULT_NAME = "Addition";
//    private static final String DEFAULT_DESCRIPTION = "Add 2 operands";
    
    private static final String DEFAULT_NAME = "XOR";
    private static final String DEFAULT_DESCRIPTION = "Logical \"Excluded OR\"";
    ...
    protected Xor(UUID id, String name, String description) {
        super(id, name, description);
    }

    protected Xor(UUID id, Xor additionToCopy) {
        super(id, additionToCopy);
    }

    protected Xor(Xor additionToCopy) {
    ...
    public Xor newInstance() {
        return new Xor(UUID.randomUUID(), this);
    }
     ...
     */
    public static Xor newTemplate() {
        UUID processorId = UUID.randomUUID();
        Xor addition = new Xor(processorId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
        ...
        // double output
        try {
            addition.setOutput(ProcessorOutput.doubleOutputWithId(OUTPUT_ID).nameAndDescription("Result").attributeName("XOR"));
    ...
    static class CompiledAddition extends CompiledProcessor<Void> {
        ...
        protected CompiledAddition(Xor addition) {
            super(addition);
