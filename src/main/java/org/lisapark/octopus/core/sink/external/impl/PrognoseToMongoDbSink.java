 ...
 */
@Persistable
public class PrognoseToMongoDbSink extends AbstractNode implements ExternalSink {
    
     private final static java.util.logging.Logger logger 
            = java.util.logging.Logger.getLogger(PrognoseToMongoDbSink.class.getName());    
    ...
    private PrognoseToMongoDbSink(UUID id, String name, String description) {
    ...
    private PrognoseToMongoDbSink(UUID id, PrognoseToMongoDbSink copyFromNode) {
    ...
    private PrognoseToMongoDbSink(PrognoseToMongoDbSink copyFromNode) {
        super(copyFromNode);
...
//    }
    ...
    @Override
    public PrognoseToMongoDbSink newInstance() {
        return new PrognoseToMongoDbSink(UUID.randomUUID(), this);
    }

    @Override
    public PrognoseToMongoDbSink copyOf() {
        return new PrognoseToMongoDbSink(this);
    }

    public static PrognoseToMongoDbSink newTemplate() {
        UUID sinkId = UUID.randomUUID();

        PrognoseToMongoDbSink gssSink = new PrognoseToMongoDbSink(sinkId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
...
//                .defaultValue(Boolean.TRUE));
    ...
    static class CompiledGssSink extends CompiledExternalSink {
        ...
        private final PrognoseToMongoDbSink gssSink;
        
        GssListUtils gssList;
        private ListFeed listFeed;
        
        protected CompiledGssSink(PrognoseToMongoDbSink gssSink) {
            super(gssSink);            
