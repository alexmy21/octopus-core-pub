 ...
 */
@Persistable
public class ProcessorJson2Neo4jSrvSink extends AbstractNode implements ExternalSink {
    
    static final Logger LOG = LoggerFactory.getLogger(ProcessorJson2Neo4jSrvSink.class);
    
    private static final String DEFAULT_NAME = "Processor Json to Neo4j Server";
    private static final String DEFAULT_DESCRIPTION = "Outputs Processor json to Rest Server Neo4j database.";
    ...
    private ProcessorJson2Neo4jSrvSink(UUID id, String name, String description) {
    ...
    private ProcessorJson2Neo4jSrvSink(UUID id, ProcessorJson2Neo4jSrvSink copyFromNode) {
        super(id, copyFromNode);
        input = copyFromNode.getInput().copyOf();
    }

    private ProcessorJson2Neo4jSrvSink(ProcessorJson2Neo4jSrvSink copyFromNode) {
        super(copyFromNode);
    ...
    @Override
    public ProcessorJson2Neo4jSrvSink newInstance() {
        return new ProcessorJson2Neo4jSrvSink(UUID.randomUUID(), this);
    }

    @Override
    public ProcessorJson2Neo4jSrvSink copyOf() {
        return new ProcessorJson2Neo4jSrvSink(this);
    }

    public static ProcessorJson2Neo4jSrvSink newTemplate() {
        UUID sinkId = UUID.randomUUID();
        
        ProcessorJson2Neo4jSrvSink neo4jSink = new ProcessorJson2Neo4jSrvSink(sinkId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
    ...
    static class CompiledModel2Neo4jSink extends CompiledExternalSink {
        ...
        private ProcessorJson2Neo4jSrvSink sink;
        private GraphDatabaseService graphDb = null; 
        Db4oNeo4jUtils utils;
                
        protected CompiledModel2Neo4jSink(ProcessorJson2Neo4jSrvSink processor) {
            super(processor);
            this.sink = processor;
        ...
        public synchronized void processEvent(SinkContext ctx, Map<Integer, Event> eventsByInputId) {
            ...
            String processorStr = (String) event.getData().get(ProcessorBean.PROC_JSON);
            String procType     = (String) event.getData().get(ProcessorBean.PROC_TYPE);
            
            LOG.info("Model JSON: " + processorStr);            
            
            if (graphDb != null && !processorStr.trim().isEmpty()) {               
                utils.addProcessor(graphDb, procType, processorStr);
                
                ctx.getStandardOut().println(processorStr);
            } else {
