 ...
 */
@Persistable
public class ProductRels2Neo4jSrvSink extends AbstractNode implements ExternalSink {
    
    static final Logger LOG = LoggerFactory.getLogger(ProductRels2Neo4jSrvSink.class);
    
    private static final String DEFAULT_NAME = "ProductRelations to Neo4j";
    private static final String DEFAULT_DESCRIPTION = "Outputs Product relations to Neo4j database.";
    ...
    private ProductRels2Neo4jSrvSink(UUID id, String name, String description) {
    ...
    private ProductRels2Neo4jSrvSink(UUID id, ProductRels2Neo4jSrvSink copyFromNode) {
        super(id, copyFromNode);
        input = copyFromNode.getInput().copyOf();
    }

    private ProductRels2Neo4jSrvSink(ProductRels2Neo4jSrvSink copyFromNode) {
        super(copyFromNode);
    ...
    @Override
    public ProductRels2Neo4jSrvSink newInstance() {
        return new ProductRels2Neo4jSrvSink(UUID.randomUUID(), this);
    }

    @Override
    public ProductRels2Neo4jSrvSink copyOf() {
        return new ProductRels2Neo4jSrvSink(this);
    }

    public static ProductRels2Neo4jSrvSink newTemplate() {
        UUID technologyStepId = UUID.randomUUID();
        
        ProductRels2Neo4jSrvSink neo4jSink = new ProductRels2Neo4jSrvSink(technologyStepId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
    ...
    public CompiledExternalSink compile() throws ValidationException {
        return new CompiledProductRels2Neo4jSink(copyOf());
    }

    static class CompiledProductRels2Neo4jSink extends CompiledExternalSink {
        
        private ProductRels2Neo4jSrvSink step;
        private GraphDatabaseService graphDb = null; 
        CpNeo4jUtils utils;
                
        protected CompiledProductRels2Neo4jSink(ProductRels2Neo4jSrvSink techStep) {
            super(techStep);
            this.step = techStep;
        ...
        public synchronized void processEvent(SinkContext ctx, Map<Integer, Event> eventsByInputId) {
            ...
            String json = (String) event.getData().get(Product.PRODUCT_JSON);
                    
            LOG.info("Model JSON: " + json);            
            
            if (graphDb != null && !json.trim().isEmpty()) {               
                Node node = utils.createProductRels(graphDb, json);
                if(node != null) {
                    utils.createProductNodeSolutionRels(graphDb, node);
                } 
