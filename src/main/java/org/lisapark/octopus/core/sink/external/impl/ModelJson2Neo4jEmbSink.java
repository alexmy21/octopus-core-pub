 ...
 */
@Persistable
public class ModelJson2Neo4jEmbSink extends AbstractNode implements ExternalSink {
    
    static final Logger LOG = LoggerFactory.getLogger(ModelJson2Neo4jEmbSink.class);
    
    private static final String DEFAULT_NAME = "Model Json to Neo4j Emb";
    private static final String DEFAULT_DESCRIPTION = "Outputs Model json to embedded Neo4j database.";
    ...
    private ModelJson2Neo4jEmbSink(UUID id, String name, String description) {
    ...
    private ModelJson2Neo4jEmbSink(UUID id, ModelJson2Neo4jEmbSink copyFromNode) {
        super(id, copyFromNode);
        input = copyFromNode.getInput().copyOf();
    }

    private ModelJson2Neo4jEmbSink(ModelJson2Neo4jEmbSink copyFromNode) {
        super(copyFromNode);
    ...
    @Override
    public ModelJson2Neo4jEmbSink newInstance() {
        return new ModelJson2Neo4jEmbSink(UUID.randomUUID(), this);
    }

    @Override
    public ModelJson2Neo4jEmbSink copyOf() {
        return new ModelJson2Neo4jEmbSink(this);
    }

    public static ModelJson2Neo4jEmbSink newTemplate() {
        UUID sinkId = UUID.randomUUID();
        
        ModelJson2Neo4jEmbSink neo4jSink = new ModelJson2Neo4jEmbSink(sinkId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
        
        neo4jSink.addParameter(
                Parameter.stringParameterWithIdAndName(NEO4J_URL_PARAMETER_ID, NEO4J_URL)
                .defaultValue("C:/Users/Alex/Neo4j/neo4j-community-1.9.RC2/data/graph.db")
    ...
    static class CompiledModel2Neo4jSink extends CompiledExternalSink {
        ...
        private ModelJson2Neo4jEmbSink sink;
        private GraphDatabaseService graphDb = null; 
        Db4oNeo4jUtils utils;
                
        protected CompiledModel2Neo4jSink(ModelJson2Neo4jEmbSink processor) {
            super(processor);
            this.sink = processor;
        ...
        public synchronized void processEvent(SinkContext ctx, Map<Integer, Event> eventsByInputId) {
            ...
            if (utils.getGraphDbService() == null) {
                utils.setGraphDbService(utils.newEmbeddedInstance(sink.getNeo4jUrl()));
            }
