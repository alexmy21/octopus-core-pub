 ...
 */
@Persistable
public class PrognoseToDatabaseSink extends AbstractNode implements ExternalSink {
    
     private final static java.util.logging.Logger logger 
            = java.util.logging.Logger.getLogger(PrognoseToDatabaseSink.class.getName());    
    
    private static final String DEFAULT_NAME        = "Вывод прогноза";
    private static final String DEFAULT_DESCRIPTION = "Вывод результатов прогнозирования в таблицу Google";
    ...
    private PrognoseToDatabaseSink(UUID id, String name, String description) {
    ...
    private PrognoseToDatabaseSink(UUID id, PrognoseToDatabaseSink copyFromNode) {
    ...
    private PrognoseToDatabaseSink(PrognoseToDatabaseSink copyFromNode) {
        super(copyFromNode);
...
//    }
    ...
    @Override
    public PrognoseToDatabaseSink newInstance() {
        return new PrognoseToDatabaseSink(UUID.randomUUID(), this);
    }

    @Override
    public PrognoseToDatabaseSink copyOf() {
        return new PrognoseToDatabaseSink(this);
    }

    public static PrognoseToDatabaseSink newTemplate() {
        UUID sinkId = UUID.randomUUID();

        PrognoseToDatabaseSink gssSink = new PrognoseToDatabaseSink(sinkId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
...
//                .defaultValue(Boolean.TRUE));
    ...
    static class CompiledGssSink extends CompiledExternalSink {
        ...
        private final PrognoseToDatabaseSink gssSink;
        
        GssListUtils gssList;
        private ListFeed listFeed;
        
        protected CompiledGssSink(PrognoseToDatabaseSink gssSink) {
            super(gssSink);            
