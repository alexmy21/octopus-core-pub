 ...
 */
@Persistable
public class XmlWebDavDataSource extends ExternalSource {
    private static final String DEFAULT_NAME        = "XML from Web DAV";
    private static final String DEFAULT_DESCRIPTION = "XML from Web Dav Server as a source for events";
    ...
    private XmlWebDavDataSource(UUID sourceId, String name, String description) {
        super(sourceId, name, description);
    }

    private XmlWebDavDataSource(UUID sourceId, XmlWebDavDataSource copyFromSource) {
        super(sourceId, copyFromSource);
    }

    private XmlWebDavDataSource(XmlWebDavDataSource copyFromSource) {
    ...
    @Override
    public XmlWebDavDataSource newInstance() {
        UUID sourceId = UUID.randomUUID();
        return new XmlWebDavDataSource(sourceId, this);
    }

    @Override
    public XmlWebDavDataSource copyOf() {
        return new XmlWebDavDataSource(this);
    }

    public static XmlWebDavDataSource newTemplate() {
        UUID sourceId = UUID.randomUUID();
        XmlWebDavDataSource jdbc = new XmlWebDavDataSource(sourceId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
        ...
        jdbc.setOutput(Output.outputWithId(1).setName("MAP_OUT"));
    ...
    private static class CompiledExcelWebDavDataSource implements CompiledExternalSource {
        private final XmlWebDavDataSource source;

        private volatile boolean running;

        public CompiledExcelWebDavDataSource(XmlWebDavDataSource source) {
            this.source = source;
        }
